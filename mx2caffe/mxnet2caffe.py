...
parser.add_argument('--mx-model',    type=str, default='../MXNet2Caffe/model_mxnet/model')
parser.add_argument('--mx-epoch',    type=int, default=30)
parser.add_argument('--cf-prototxt', type=str, default='./model_caffe/model.prototxt')
parser.add_argument('--cf-model',    type=str, default='./model_caffe/model.caffemodel')
...
for i_key,key_i in enumerate(all_keys):
  ...
  try:    
    ...
    # elif '_gamma' in key_i and 'relu' not in key_i:     # for r100 ect al.
    elif '_gamma' in key_i and '_act' not in key_i:   # for vargface
      key_caffe = key_i.replace('_gamma','_scale')
      net.params[key_caffe][0].data.flat = arg_params[key_i].asnumpy().flat
    # TODO: support prelu
    # elif '_gamma' in key_i and 'relu' in key_i:   # for r100 ect al.
    elif '_gamma' in key_i and '_act' in key_i:   # for vargface
