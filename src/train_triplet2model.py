...
def parse_args():
  ...
  parser.add_argument('--supermodel', default='', help='big model to load')
  ...
  parser.add_argument('--target', type=str, default='lfw,casia_ivs,ja_rd_ivs', help='')
...
def get_symbol(args, arg_params, aux_params, sym_embedding=None):
  if sym_embedding is None:
    ...
    elif args.network[0] =='g':
      print('init globalRE resnet 152 from yunpeng', args.num_layers)
      embedding = fresnetGRN.get_symbol(args.emb_size,
                  version_se=args.version_se, version_input=args.version_input,
                  version_output=args.version_output, version_unit=args.version_unit)
    else:
...
def train_net(args):
    ...
    super_model.bind( data_shapes=[('data', (args.batch_size, 3, image_size[0], image_size[1]))] )
    super_model.set_params(super_arg_params, super_aux_params)
