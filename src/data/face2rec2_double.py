...
def my_pack(header, s):
    flag = header.flag
    lable = header.label[0]
    id = header.id
    id2 = header.id2
    s_header = struct.pack(_IR_FORMAT, flag,lable, id, id2)
    s_flag = ''
    if header.flag > 0:
        flag = header.flag
        label = header.label
        # print("label:",label)
        # print("header:",header)
        s_flag = struct.pack('dd',label[0],label[1])
    s = s_header + s_flag + s
...
def read_list(path_in):
    with open(path_in) as fin:
        ...
        item.flag = 2
        item.id = 0
        # item.label = [double(_id), double(_id+len(identities))]
        label = np.array([_id, _id+len(identities)], dtype=np.double)
        item.label = label
        yield item
        for identity in identities:
          ...
          item.id = _id
          _id+=1
          label = np.array([identity[0],identity[1]], dtype=np.double)
          item.label = label
          yield item
