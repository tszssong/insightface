...
class FaceImageIter(io.DataIter):
    ...
    def __init__(self, batch_size, data_shape,
        ...
        print(data_shape, data_shape[0], data_shape[1])
        if config.dream:
            self.provide_data = [(data_name, (batch_size,) + data_shape[0]), 
                                 ("yaw", (batch_size,1))]
        else:
            self.provide_data = [(data_name, (batch_size,) + data_shape[0])]
        ...
        self.image_size = '%d,%d'%(data_shape[0][1],data_shape[0][2])
    ...
    def provide_data(self):
        if config.dream:
            return [("data",(self.batch_size,)+self.data_shape[0]), ("yaw",(self.batch_size,)+(1,))]
        else:
            return [("data",(self.batch_size,)+self.data_shape[0])]
   
    def provide_label(self):
        return [("sofmax_label", (self.batch_size,))]
    ...
    def next(self):
        ...
        c, h, w = self.data_shape[0]
        batch_data = nd.empty((batch_size, c, h, w))
        batch_yaw = nd.empty((batch_size, 1))
        ...
        try:
            while i < batch_size:
                ...
                if _data.shape[0]!=self.data_shape[0][1]:
                  _data = mx.image.resize_short(_data, self.data_shape[0][1])
                if self.rand_mirror:
                  _rd = random.randint(0,1)
                  if _rd==1:
        ...
        return io.DataBatch([batch_data, batch_yaw], [batch_label], batch_size - i)
...
if __name__=='__main__':
    print ("read rec2img")
    save_dir = '/home/zhengmeisong/TrainData/glintv2_demo/data/'
    ...
    train_dataiter = FaceImageIter(
          ...
          data_shape           = [(3, 112, 112),(1)],
          path_imgrec          = '/home/zhengmeisong/TrainData/glintv2_angle/train.rec',
          shuffle              = True,
          rand_mirror          = False,
          mean                 = None,
          cutoff               = False,
    ...
    for i in range(num_of_img):
        ...
        print(type(batch))
        print(type(batch.data), type(batch.data[0]), type(batch.data[1]))
