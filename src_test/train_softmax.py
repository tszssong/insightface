...
def get_symbol(args, arg_params, aux_params):
  ...
  if args.network[0]=='d':
    ...
    embedding = fmobilefacenet.get_symbol_v6(args.emb_size, bn_mom = args.bn_mom, version_output=args.version_output)
  else:
...
def train_net(args):
    ...
    def ver_test(nbatch):
      ...
      for i in xrange(len(ver_list)):
        acc1, std1, eer, acc2, std2, xnorm, embeddings_list = verification.test(ver_list[i], model, args.batch_size, 10, None, None)
        print('[%s][%d]XNorm: %f EER: %f' % (ver_name_list[i], nbatch, xnorm, eer))
